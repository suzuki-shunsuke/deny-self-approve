...
complete -o bashdefault -o default -o nospace -F _cli_bash_autocomplete deny-self-approve`)
	return nil
}
...
if [ "$funcstack[1]" = "_deny_self_approve" ]; then
  _deny_self_approve "$@"
else
  compdef _deny_self_approve deny-self-approve
fi`)
	return nil
}
